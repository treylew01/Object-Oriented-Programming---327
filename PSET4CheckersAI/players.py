...
class MinimaxPlayer(Player):
    ...
    def take_turn(self, game_state):
        ...
        for move in total_moves:
            if self.side == WHITE:
                if str(move._start) == "b8" or str(move._start) == "d8" or  str(move._start) == "f8" or  str(move._start) == "h8":
                    end_piece_moves.append(move)
                else:
                    other_moves.append(move)
            else:
                if str(move._start) == "a1" or str(move._start) == "c1" or  str(move._start) == "e1" or  str(move._start) == "g1":
                    end_piece_moves.append(move)
                else:
                    other_moves.append(move)
        ...
        if len(other_moves) > 1:
            for move in other_moves:
                ...
                if score > current_score:
                    winning_move = move
                    current_score = score
        elif len(end_piece_moves) >= 1:
            for move in end_piece_moves:
                move.execute(game_state)
                score = self.minimax(game_state, self.depth - 1, -9999, 9999)
                move.undo(game_state)
                if score > current_score:
